...
export default function App(){
  ...
  function processMain(opt){
    ...
    if(mainStep<MAIN_Q.length-1) setMainStep(mainStep+1);
      ...
      setResult(r);
      // auto-update profile
      setProfile(p=>({...p,skinType:deriveSkinType(skinAns),hairType:deriveHairType(hairAns),hairDensity:deriveHairDensity(hairAns)}));
      setScreen("result");
    }
  }
...
${moodItems.length?`Mood board: ${moodItems.join(", ")}`:""}`;
  ...
  const nav=[["Quiz","quiz"],["Stylist","chat"],["Profile","profile"],["Timeline","timeline"],["Products","products"],["Artists","directory"]];

  return(
    <div style={{minHeight:"100vh",background:C.white,fontFamily:co,color:C.black}}>

      {/* NAV */}
      {screen!=="home"&&(
        <div style={{position:"sticky",top:0,zIndex:100,background:C.white,borderBottom:`0.5px solid ${C.border}`,padding:"9px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
